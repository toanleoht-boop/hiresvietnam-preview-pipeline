...
for (const [index, track] of job.tracks.entries()) {
  ...
  const ffmpegArgs = [
    ...
    '-ss', String(track.previewStart ?? job.previewStart ?? 0),
    '-i', sourcePath,
    '-vn', '-map_metadata', '-1', '-codec:a', 'libmp3lame', '-b:a', String(job.bitrate || '160k'),
    outputPath
  ];
  const previewDuration = Number(track.previewDuration ?? job.previewDuration ?? 0);
  if (previewDuration > 0) ffmpegArgs.splice(8, 0, '-t', String(previewDuration));
